...
#
#      modified by Ron Lee <ron@debian.org> 3/4/99
...
$com = $0;
$com =~ s/.*\///;
$usage = "Usage: $com [-a][-c|-C][-f][-F][-l][-n][-v][-w][-d tagdir][-S cgidir][-t title][dir]";
...
Options:
     ...
     -C,
             as for -c but use html.gz suffix.
     -f, --form
...
if ($'cflag && !&'usable('gzip')) {
	print STDERR "Warning: 'gzip' command not found. -$'cflag option ignored.\n";
	$'cflag = '';
}
...
if ($'cgi && $'cflag eq 'c') {
	&makehtaccess("$dist/.htaccess.skel") || &'error("cannot make .htaccess skeleton.");
	chmod(0644, "$dist/.htaccess.skel") || &'error("cannot chmod .htaccess skeleton.");
	if ($'cgidir) {
